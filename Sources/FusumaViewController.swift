...
public var fusumaTitleColor         = UIColor.hex("#c9c7c8", alpha: 1.0)
public var fusumaTintColor          = UIColor.hex("#424141", alpha: 1.0)
public var fusumaBackgroundColor    = UIColor.hex("#FCFCFC", alpha: 1.0)
public var fusumaTopBarButtonColor  = UIColor.hex("#424141", alpha: 1.0)
...
@objc public class FusumaViewController: UIViewController {
    ...
    override public func viewDidLoad() {
        ...
        closeButton.tintColor   = fusumaTopBarButtonColor
        doneButton.tintColor    = fusumaTopBarButtonColor
        ...
        titleLabel.textColor = fusumaTitleColor
    ...
    override public func viewDidAppear(_ animated: Bool) {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + .milliseconds(1)) { // work around for the 1 sec freeze view
        
            if self.availableModes.contains(.camera) {
                
                self.albumView.frame = CGRect(origin: CGPoint.zero, size: self.photoLibraryViewerContainer.frame.size)
            ...
            if self.availableModes.contains(.camera) {
                ...
                self.cameraView.frame = CGRect(origin: CGPoint.zero, size: self.cameraShotContainer.frame.size)
                self.cameraView.layoutIfNeeded()
                self.cameraView.initialize()
            }
            
            if self.availableModes.contains(.video) {
                
                self.videoView.frame = CGRect(origin: CGPoint.zero, size: self.videoShotContainer.frame.size)
